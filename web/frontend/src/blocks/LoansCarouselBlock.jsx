...
import { ProductCarouselTopBar } from '../components/ProductCarouselTopBar';
import { ProductCarouselScrollButton } from '../components/ProductCarouselScrollButton';

export function LoansCarouselBlock({
  sectionHeader = 'ACHIEVE YOUR GOALS WITH OUR FLEXIBLE LENDING OPTIONS',
  buttonText = 'LOANS',
  linkText = 'ALL LOAN PRODUCTS',
  linkUrl = '/loan-products',
  categoryId = 0,
  maxItems = 9,
  ...
  const scrollClipId = `clip0_scroll_button_loans_carousel_${useId().replace(/:/g, '')}`;
  ...
  useEffect(() => {
    ...
    fetchLoanProductsList({ categoryId: Number(categoryId) || 0, limit: lim })
      .then((data) => {
        if (cancelled) return;
      ...
      .catch((err) => {
        ...
        console.error('[LoansCarouselBlock] fetch failed', err);
  ...
  return (
    ...
    >
      <ProductCarouselTopBar
        sectionBgColor={sectionBgColor}
        topBarColor={topBarColor}
        topBarGradientAngle={topBarGradientAngle}
        topBarGradientFrom={topBarGradientFrom}
        topBarGradientVia={topBarGradientVia}
        topBarGradientTo={topBarGradientTo}
      />

      <div className="mx-auto max-w-7xl px-4" style={{ animation: 'fadeInUp 0.8s ease-out' }}>
        <ProductCarouselScrollButton
          clipId={scrollClipId}
          outerFill={arrowButtonBgColor}
          innerFill={arrowButtonIconColor}
        />
        ...
        <div className={PRODUCT_CAROUSEL_STAGE_CLASS}>
          <ProductCarouselNavButton direction="prev" onClick={scrollPrev} ariaLabel="Previous loans" />
          <ProductCarouselNavButton direction="next" onClick={scrollNext} ariaLabel="Next loans" />

          <div className={PRODUCT_CAROUSEL_VIEWPORT_CLASS} ref={emblaRef}>
            <div className={PRODUCT_CAROUSEL_TRACK_CLASS}>
              {isLoading
                ? Array.from({ length: 6 }).map((_, i) => (
                    <ProductCarouselCardSkeleton key={`loan-carousel-skeleton-${i}`} />
                  ))
                : items.map((card) => (
                    <div key={card.id || card.slug || card.title} className={PRODUCT_CAROUSEL_SLIDE_CLASS}>
                      <ProductCarouselCard
                        ...
                        imageAlt={card.title || 'Loan product'}
                      />
                    </div>
                  ))}
            </div>
