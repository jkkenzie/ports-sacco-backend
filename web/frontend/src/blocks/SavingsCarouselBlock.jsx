...
import { fetchSavingsProductsList } from '../api/savingsProducts';
import {
  ProductCarouselCard,
...
export function SavingsCarouselBlock({
  sectionHeader = 'DISCOVER OUR SAVINGS SOLUTIONS',
  buttonText = 'SAVINGS',
  linkText = 'ALL SAVINGS PRODUCTS',
  linkUrl = '/savings-products',
  maxItems = 9,
  ...
  useEffect(() => {
    ...
    fetchSavingsProductsList({ limit: lim })
      .then((data) => {
        if (cancelled) return;
      ...
      .catch((err) => {
        ...
        console.error('[SavingsCarouselBlock] fetch failed', err);
      ...
      <div className="mx-auto max-w-7xl px-4" style={{ animation: 'fadeInUp 0.8s ease-out' }}>
        ...
        <div className={PRODUCT_CAROUSEL_STAGE_CLASS}>
          <ProductCarouselNavButton
            direction="prev"
            onClick={scrollPrev}
            ariaLabel="Previous savings products"
          />
          <ProductCarouselNavButton
            direction="next"
            onClick={scrollNext}
            ariaLabel="Next savings products"
          />
            ...
            <div className={PRODUCT_CAROUSEL_TRACK_CLASS}>
              {isLoading
                ? Array.from({ length: 6 }).map((_, i) => (
                    <ProductCarouselCardSkeleton key={`savings-carousel-skeleton-${i}`} />
                  ))
                : items.map((card) => (
                    <div key={card.id || card.slug || card.title} className={PRODUCT_CAROUSEL_SLIDE_CLASS}>
                      <ProductCarouselCard
                        ...
                        imageAlt={card.title || 'Savings product'}
                      />
                    </div>
                  ))}
            </div>
