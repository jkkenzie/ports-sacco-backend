...
export function SavingsProductsCardsSection() {
  ...
  useEffect(() => {
    ...
    const loadProducts = async () => {
      try {
        const data = await fetchSavingsProductsList({ limit: 0 });
        if (!isCancelled) {
        ...
        console.error('[SavingsProductsCardsSection] Load failed', err);
        if (!isCancelled) {
          setProducts([]);
          setError(SAVINGS_PRODUCTS_ERROR_MESSAGE);
        }
      } finally {
        if (!isCancelled) {
          setIsLoading(false);
  ...
  return (
    <section className="w-full py-6 lg:py-6">
      <div className=" mx-auto" style={{ animation: 'fadeInUp 0.8s ease-out' }}>
        {!isLoading && error ? (
          <p className="text-[#ee6e2a] text-base px-4">{error}</p>
        ) : null}
        {!isLoading && !error && products.length === 0 ? (
          <p className="text-[#3b4e6b] text-base px-4">No savings products available right now.</p>
        ) : null}
