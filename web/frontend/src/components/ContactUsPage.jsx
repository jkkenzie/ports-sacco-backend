...
const CONTACT_US_PAGE_ERROR_MESSAGE = 'Something went wrong. Please try again later.';
...
export function ContactUsPage() {
  ...
  useEffect(() => {
    ...
    fetchPageBySlug('contact-us')
      .then((result) => {
        ...
        if (!result.ok) {
          console.error('[ContactUsPage] Page fetch failed', {
            status: result.status,
            slug: 'contact-us',
          });
          setBlocks([]);
          setStatus({ loading: false, error: CONTACT_US_PAGE_ERROR_MESSAGE, notFound: false });
          return;
        }
      ...
      .catch((e) => {
        if (cancelled) return;
        console.error('[ContactUsPage] Page fetch errored', e);
        setBlocks([]);
        setStatus({ loading: false, error: CONTACT_US_PAGE_ERROR_MESSAGE, notFound: false });
      });
  ...
  const heroIndex = Array.isArray(blocks)
    ? blocks.findIndex((block) => block?.name === 'custom/savings-archive-hero')
    : -1;
