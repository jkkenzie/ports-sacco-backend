...
const EVENTS_PAGE_ERROR_MESSAGE = 'Something went wrong. Please try again later.';
  ...
  useEffect(() => {
    ...
    fetchPageBySlug('events')
      .then((result) => {
        ...
        if (!result.ok) {
          console.error('[EventsPage] Page fetch failed', {
            status: result.status,
            slug: 'events',
          });
          setBlocks([]);
          setStatus({ loading: false, error: EVENTS_PAGE_ERROR_MESSAGE, notFound: false });
      ...
      .catch((e) => {
        if (cancelled) return;
        console.error('[EventsPage] Page fetch errored', e);
        setBlocks([]);
        setStatus({ loading: false, error: EVENTS_PAGE_ERROR_MESSAGE, notFound: false });
      });
