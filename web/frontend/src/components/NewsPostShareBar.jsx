...
import { buildNewsPostShareUrl, buildShareLinks } from '../utils/shareUrl';
...
export function NewsPostShareBar({ post }) {
  const [copyStatus, setCopyStatus] = useState('idle');

  const shareUrl = useMemo(() => buildNewsPostShareUrl(post), [post]);
  ...
  const links = useMemo(
    ...
    [shareUrl, share.title, share.description, post?.title, post?.excerpt]
  );

  const openShare = (platformId) => {
    if (platformId === 'instagram') {
      const text = links.copyText;
      ...
      window.prompt('Copy this link to share on Instagram:', text);
      return;
    }
  ...
  return (
    ...
    >
      ...
      {copyStatus === 'copied' ? (
        <p className="mt-2 text-sm font-medium text-[#22acb6]">Link copied — paste it in Instagram.</p>
      ) : null}
      {copyStatus === 'error' ? (
        <p className="mt-2 text-sm text-red-600">Could not copy the link. Please copy it manually above.</p>
      ) : null}
    </section>
  );
}
