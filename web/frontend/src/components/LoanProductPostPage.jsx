...
export function LoanProductPostPage() {
  ...
  useEffect(() => {
    ...
    const customUrl = customApiUrl(`/loan-products/${encodeURIComponent(currentSlug)}`);
    const fallbackUrl = wpApiUrl(`/wp-json/wp/v2/loan-products?slug=${encodeURIComponent(currentSlug)}&_embed&per_page=1`);
    ...
    Promise.allSettled([
      ...
      .then(async (results) => {
        ...
        console.error('[LoanProductPostPage] Both endpoints failed', {
          slug: currentSlug,
        ...
        setPost(null);
        setStatus({ loading: false, error: LOAN_PRODUCTS_ERROR_MESSAGE, notFound: false });
      })
      .catch((e) => {
        if (cancelled) return;
        console.error('[LoanProductPostPage] Load errored', e);
        setPost(null);
        setStatus({ loading: false, error: LOAN_PRODUCTS_ERROR_MESSAGE, notFound: false });
      });
  ...
  if (status.notFound) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        <h1 className="text-2xl font-semibold text-[#1f0026] mb-2">Loan product not found</h1>
      </div>
    );
  }
        ...
        <main className="relative z-10 bg-transparent max-w-7xl mx-auto w-full pb-0 pt-0 mt-[40px] mb-16 ">
          {otherBlocks.length > 0 ? (
            ...
            <div className="w-full bg-white py-8 px-3 rounded-2xl text-neutral-500 text-sm">
              No blocks on this loan product yet. Add Gutenberg blocks to its content.
            </div>
          ) : null}
        </main>
      </div>
