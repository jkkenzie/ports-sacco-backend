...
import { fetchEventPost } from '../api/events';
import { BlockRenderer } from '../blocks/BlockRenderer';
import { useSocialMeta } from '../hooks/useSocialMeta';
import { buildEventPostShareUrl } from '../utils/shareUrl';
import { ImageWithFallback } from './figma/ImageWithFallback';
import { EventPostNavigation } from './EventPostNavigation';
import { EventPostShareBar } from './EventPostShareBar';
import { splitPageHeroBlocks } from './StandardPageLayout';

const EVENT_POST_ERROR = 'Failed to load this event. Please try again later.';
...
export function EventPostPage() {
  ...
  useEffect(() => {
    ...
    fetchEventPost(currentSlug)
      ...
      .catch((e) => {
        ...
        console.error('[EventPostPage] Load failed', e);
        ...
        setPost(null);
        setStatus({ loading: false, error: EVENT_POST_ERROR, notFound: false });
      });
  ...
  const shareMeta = useMemo(() => {
    ...
    return {
      ...
      url: buildEventPostShareUrl(post),
    };
  }, [post]);
  ...
  if (status.notFound) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        <h1 className="text-2xl font-semibold text-[#1f0026] mb-2">Event not found</h1>
        <Link to="/events" className="text-[#22acb6] underline text-sm mt-4 inline-block">
          Back to events
        </Link>
      </div>
    );
    ...
    <>
      ...
      <div style={{ backgroundColor: '#f3f5f7' }}>
        <main className="relative z-10 mx-auto w-full max-w-4xl pb-16 pt-8 px-4 sm:px-6">
          <nav className="mb-6 flex flex-wrap items-center gap-1 text-sm text-[#64748b]" aria-label="Breadcrumb">
            <Link to="/" className="transition-colors hover:text-[#22acb6]">
              Home
            </Link>
            <ChevronRight className="size-4 shrink-0" aria-hidden />
            <Link to="/events" className="transition-colors hover:text-[#22acb6]">
              Events
            </Link>
            <ChevronRight className="size-4 shrink-0" aria-hidden />
            <span className="truncate font-medium text-[#1e293b]">{post?.title}</span>
          </nav>
          ...
          <article className="overflow-hidden rounded-3xl border-2 border-[#e8e8e8] bg-white shadow-[0_12px_40px_rgba(15,23,42,0.06)]">
            ...
            <div className="p-6 sm:p-10">
              ...
              <h1
                className="mb-4 text-3xl font-black leading-tight text-[#1e293b] sm:text-4xl"
                style={{ fontFamily: 'Museo, Helvetica, sans-serif' }}
              >
                {post?.title}
              </h1>

              <div className="mb-8 flex flex-wrap gap-4 border-b border-[#e2e8f0] pb-6 text-sm text-[#64748b]">
                {post?.author ? (
              ...
              {!contentBlocks.length && post?.excerpt ? (
                <p className="mb-8 text-lg leading-relaxed text-[#3b4e6b]">{post.excerpt}</p>
              ) : null}
              ...
              {contentBlocks.length > 0 ? (
                ...
                <p className="text-sm text-neutral-500">No content for this event yet.</p>
              ) : null}

              <EventPostShareBar post={post} />
            </div>
          </article>

          <EventPostNavigation navigation={post?.navigation} />
        </main>
      </div>
    </>
  );
