...
export function ServicesCardsSectionCpt({ categoryId = 0 }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        console.error('[ServicesCardsSectionCpt] Load failed', err);
        if (!isCancelled) {
          setItems([]);
          setError(SERVICES_ERROR_MESSAGE);
        }
      } finally {
    ...
    <section className="w-full py-6 px-6 lg:py-6">
      <div className="mx-auto" style={{ animation: 'fadeInUp 0.8s ease-out' }}>
        ...
        {!isLoading && !error && items.length === 0 ? (
          <p className="text-[#3b4e6b] text-base px-4">No services available right now.</p>
        ) : null}
