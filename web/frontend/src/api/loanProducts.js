...
const CUSTOM = customApiUrl('/loan-products');
const FALLBACK = wpApiUrl('/wp-json/wp/v2/loan-products');
...
/**
 * Fetch loan products: tries custom endpoint first (fast + cached server-side), then WP v2.
 * Uses short-lived in-memory cache and request deduplication.
 *
 * @param {{ categoryId?: number, limit?: number }} opts
 * @returns {Promise<Array<object>>}
 */
export async function fetchLoanProductsList({ categoryId = 0, limit = 0 } = {}) {
  ...
  const promise = (async () => {
    ...
    try {
      ...
      console.error('[loanProducts] custom endpoint failed', e);
    }
