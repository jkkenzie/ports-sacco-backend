(function (blocks, blockEditor, components, data, element, i18n) {
  ...
  registerBlockType('custom/loan-products-grid', {
    apiVersion: 3,
    title: __('Loan Products Grid', 'headless-core'),
    icon: 'screenoptions',
    category: 'widgets',
    description: __('Displays all Loan Products posts as cards in the frontend.', 'headless-core'),
    attributes: {
      categoryId: { type: 'number', default: 0 },
    },
    edit: function (props) {
      var blockProps = useBlockProps({ className: 'headless-loan-products-grid-block' });
      ...
      return el(
        ...
        el(
          ...
          el('strong', null, __('Loan Products Grid', 'headless-core')),
          el(
            'p',
            { style: { marginTop: '8px', marginBottom: 0, color: '#555' } },
            selectedId > 0
              ? __('Frontend will render loan products for the selected category.', 'headless-core')
              : __('Frontend will fetch and render all loan products using the existing card styling.', 'headless-core')
          )
        )
      );
    },
