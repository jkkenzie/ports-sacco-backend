(function (blocks, blockEditor, components, data, element, i18n) {
  ...
  registerBlockType('custom/services-grid', {
    apiVersion: 3,
    title: __('Services Grid', 'headless-core'),
    icon: 'screenoptions',
    category: 'widgets',
    description: __('Displays all Services posts as cards.', 'headless-core'),
    attributes: {
      categoryId: { type: 'number', default: 0 },
    },
    edit: function (props) {
      var blockProps = useBlockProps({ className: 'headless-services-grid-block' });
      ...
      return el(
        ...
        el(
          ...
          el('strong', null, __('Services Grid', 'headless-core')),
          el(
            'p',
            { style: { marginTop: '8px', marginBottom: 0, color: '#555' } },
            selectedId > 0
              ? __('Frontend will render services for the selected category.', 'headless-core')
              : __('Frontend will fetch and render all services as cards.', 'headless-core')
          )
        )
      );
    },
